...
#define R 2 // [수정] 행을 2로 설정 (5쌍을 위해 총 10칸)
#define C 5 // 열
#define PAIRS 5 // [수정] 짝의 개수를 5개로 설정 (총 10장의 카드)
...
int main(void)
{
    ...
    while (attempts > 0 && score < PAIRS)
    {
        ...
        get_selection(&r1, &c1);
        printf("첫 번째 선택된 카드 : %c\n", cards[r1][c1]); // cards로 수정

        get_selection(&r2, &c2);
        while (r1 == r2 && c1 == c2)
        {
            printf("같은 카드를 선택했습니다. 다시 선택하세요.\n");
            get_selection(&r2, &c2); // get_selection으로 수정
        }
        printf("두 번째 선택된 카드 : %c\n", cards[r2][c2]);

        // if문 뒤 세미콜론(;) 제거. else 오류 해결
